...
time_range <-
  seq(as.Date("2022-07-01"), as.Date("2022-09-30"), by = "day")
...
process_device_id <- function(device_id, date, state) {
  ...
  if (status_code(raw_data) != 200) {
    # Skip processing if the response is not successful (status code other than 200)
    return()
  }
  ...
  if (!grepl("application/json", content_type, ignore.case = TRUE)) {
    ...
    # ... Perform necessary actions with the raw response ...
    
    # Example: Write the error message and raw response to a log file
  ...
  # Save data to CSV file named after the device ID only if the data is not empty
  ...
  csv_file_path <-
    file.path(device_folder_path, paste0(device_id, "-", date, "-", state, ".csv"))
