...
monthly_averages$pollutant <- factor(monthly_averages$pollutant, levels = c("pm1", "pm25", "pm10"))

# Assign colors to each pollutant
color_palette <- wes_palette(n = 3, name = "GrandBudapest1", type = "continuous")
color_mapping <- c("pm1" = color_palette[2], "pm25" = color_palette[3], "pm10" = color_palette[1])

# Create separate graphs for each month and sensor
monthly_graphs <- lapply(unique(monthly_averages$month), function(m) {
