...
pm_subset_long$pollutant <- factor(pm_subset_long$pollutant, levels = c("pm10", "pm25", "pm1"))

# Assign colors to each pollutant
color_palette <- wes_palette(n = 3, name = "GrandBudapest1", type = "continuous")
color_mapping <- c("pm10" = color_palette[1], "pm25" = color_palette[3], "pm1" = color_palette[2])

# Create a list to store the monthly figures
monthly_figures <- list()

# Create separate graphs for each month and sensor
for (m in unique(monthly_averages$month)) {
  month_data <- monthly_averages[monthly_averages$month == m, ]
  
  # Create the bar graph with modified stack order and hover text
  p <- ggplot(month_data, aes(x = sn, y = value, fill = pollutant, text = paste(pollutant, ": ", value))) +
  ...
  # Convert the ggplot to plotly figure
  p <- ggplotly(p)
  
  # Add the plotly figure to the list
  monthly_figures[[m]] <- p
}
...
# Create the bar graph with hover text
p <- ggplot(yearly_sums, aes(x = sn, y = value, fill = pollutant, text = paste(pollutant, ": ", value))) +
  geom_col(position = "stack") +
  labs(x = "Sensor ID", y = "ug/m3") +
