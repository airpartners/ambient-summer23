...
This document primarily focuses on importing, merging, and pre-processing the data to prepare it for further analysis. The result of

Follow the **FETCHING DATA TO IMPORT** session to retrieve data for this document. Pressing `Ctrl + Alt + R` (run all) for this markdown file would results in a `graphableData.RData` file being created and saved into the data folder. This file contains the data frame `mod_met` that can be used to graph, to visualize this data frame, simply load it and run the print function commented below:
...
To understand and modify the below code, see the **FILE GUIDE** session below.

## FILE GUIDE

A. **INITIAL SETTINGS**

-   Required R packages are installed and loaded. Warnings during package loading are suppressed.

B. **FETCHING DATA TO IMPORT**

-   As stated before, in order to run this markdown file, you need a few file pre-requisites fulfilled. The instructions to retrieve them is found in this session.

C. **IMPORT DATA AND PRE-PROCESSING**

1.  Quant-AQ Data

-   Import data from .csv files by columns, clean up names, add latitude, longitude, site names, cleaned data, and store in the data frame `mod`

2.  Meteorology Data

-   Import data from all .csv files in the `Metdata` folder, clean data, and store in the data frame `metdata`

D. **DATA PROCESSING**

1.  Merging Datasets: Time sync and merge the Quant-AQ and Meteorology datasets

2.  Cleaning Up and Save the Work Space: I think the name is pretty descriptive

3.  Sanity Checks: Optional commented out code that you can use to test if the code above is working correctly
...
The meteorology data used in our analysis is taken from [Iowa Environmental Mesonet](https://mesonet.agron.iastate.edu/request/download.phtml?network=MA_ASOS).
...
After retrieving the data, go to `ambient-summer23/data_acquisition/data_breakdown.Rmd` and run the first and third code blocks to break the meteorology data down by monthly `.csv` files.
...
modPM_cols <- c("pm1", "pm25", "pm10", "opcbin0", "opcbin1", "opcbin2", "opcbin3", "opcbin4", "opcbin5", "opcbin6", "opcbin7", "opcbin8", "opcbin9", "opcbin10", "opcbin11", "opcbin12", "opcbin13", "opcbin14", "opcbin15", "opcbin16", "opcbin17", "temp", "timestamp_local", "sn.x", "timestamp.x",)

modulair_cols <- c("pm1", "pm25", "pm10", "co", "no", "no2", "o3", "opc.bin0", "opc.bin1", "opc.bin2", "opc.bin3", "opc.bin4", "opc.bin5", "opc.bin6", "opc.bin7", "opc.bin8", "opc.bin9", "opc.bin10","opc.bin11", "opc.bin12", "opc.bin13", "opc.bin14", "opc.bin15", "opc.bin16", "opc.bin17", "temp", "timestamp_local", "sn.x", "timestamp.x")
  ...
  data <- modPM_devices %>%
    map_dfr(function(modPM_device) {
      paths <- c(
        ...
        sprintf(
          "../data/Quant-AQ/MOD-PM/broken-down/%s/%s-2023-%02d.csv",
          modPM_device, modPM_device, 1:2
        )
      )
  ...
  data <- modulair_devices %>%
    map_dfr(function(modulair_device) {
      paths <- c(
        ...
        sprintf(
          "../data/Quant-AQ/MOD-PM/broken-down/%s/%s-2023-%02d.csv",
          modulair_device, modulair_device, 1:2
        )
      )
      map_dfr(paths, function(path) {
        suppressWarnings(
