...
-   Import data from .csv files by columns, clean up names, add latitude, longitude, site names, cleaned data, and store in the data frame `modPM`
...
columns_to_read <- c(
  "pm1", "pm25", "pm10", "co", "no", "no2", "o3", "opc.bin0",
  "opc.bin1", "opc.bin2", "opc.bin3", "opc.bin4", "opc.bin5",
  "opc.bin6", "opc.bin7", "opc.bin8", "opc.bin9", "opc.bin10",
  "opc.bin11", "opc.bin12", "opc.bin13", "opc.bin14", "opc.bin15",
  "opc.bin16", "opc.bin17", "timestamp_local", "sn.x", "timestamp.x"
)

# Read in csv files from data folder and select specific columns
device_ids <- c(
  "MOD-00024",
  "MOD-00025",
  "MOD-00026",
    ...
    map_dfr(function(sensor_id) {
      paths <- c(
        ...
        sprintf(
          ...
          sensor_id, sensor_id, 1:2
        )
      )
      map_dfr(paths, function(path) {
        suppressWarnings(
          tryCatch(
            read_csv(path, col_select = columns_to_read),
