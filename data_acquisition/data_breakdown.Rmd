...
can also rewrite `API_call.Rmd` to not merge monthly files.
...
source("data_breakdown_helpers.R")
...
Refer to the Markdown file API_call.Rmd in this folder. The data should
be in ./data/Quant-AQ/MOD-PM/ if nothing goes wrong.
...
is in this folder and the cell in `PACKAGES AND INITIAL SETTINGS` is
ran.
...
for (sensor_id in device_ids) {
  ...
  output_folder <-
    paste0("./data/Quant-AQ/MOD-PM/broken-down/", sensor_id)
  # Load data
...
colnames(data)[valid] <- "timestamp_local" # Name change to fit with function
name <- "metdata"
# Breakdown csv
breakdown_csv(data, name, start_date, end_date)
```
